...
// Empty array for storing GDP data for each country
let gdpData = [];
...
function handleSearch(event) {
  ...
  if (searchTerm) {
    // Filtering the countriesData array to create a new array containing only countries whose name includes the search term
    const filteredCountries = countriesData.filter((country) =>
      //Searching for a country name that includes the search term
...
function filterByRegionAndMisc() {
  ...
  } else if (selectedMiscFilter === "highest-gdp") {
    // Sort by descending GDP
    filteredCountries.sort((a, b) => b.gdp - a.gdp);
  } else if (selectedMiscFilter === "lowest-gdp") {
    // Sort by ascending GDP
    filteredCountries.sort((a, b) => a.gdp - b.gdp);
  }
