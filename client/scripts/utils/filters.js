 ...
 */
export function filterCountriesBySearchTerm(countries, searchTerm) {
    if (!searchTerm) {
        return countries;
    }
    const lowerCaseSearchTerm = searchTerm.toLowerCase();
    return countries.filter((country) => country.name.toLowerCase().includes(lowerCaseSearchTerm));
}
/**
 ...
 */
export function updateCountryCounter(count) {
    const countryCounterElement = document.getElementById("country-counter");
    if (countryCounterElement) {
        countryCounterElement.textContent = `${count} ${count === 1 ? "Country" : "Countries"}`;
    }
}
/**
 ...
 */
export function populateRegionFilter(countries) {
